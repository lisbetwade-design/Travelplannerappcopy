...
-- Create time_off table to store user time off dates
...
-- Create RLS policies for time_off table
CREATE POLICY "Users can view their own time off" ON public.time_off
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own time off" ON public.time_off
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own time off" ON public.time_off
  FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users can delete their own time off" ON public.time_off
  FOR DELETE USING (auth.uid() = user_id);
